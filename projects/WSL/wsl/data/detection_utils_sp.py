...
def transform_proposals_seg(
    ...
    """
    if "proposal_file" in dataset_dict:
        proposal_file = dataset_dict["proposal_file"]
        with PathManager.open(proposal_file, "rb") as f:
            proposals = pickle.load(f, encoding="latin1")

        superpixels = proposals["superpixels"]
        oh_labels = proposals["oh_labels"]
        scores = np.squeeze(proposals["scores"])
        boxes = proposals["boxes"]
        assert proposals["indexes"] == dataset_dict["image_id"]

        bbox_mode = (
            BoxMode(proposals["bbox_mode"]) if "bbox_mode" in proposals else BoxMode.XYXY_ABS
        )

        dataset_dict["proposal_boxes"] = boxes
        dataset_dict["proposal_objectness_logits"] = scores
        dataset_dict["proposal_bbox_mode"] = bbox_mode
        ...
        oh_labels = torch.as_tensor(oh_labels.astype("int32"))
