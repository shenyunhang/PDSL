...
__all__ = ["fast_rcnn_inference", "WSDDNOutputLayers"]
...
class WSDDNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, gt_classes_img_oh):
        ...
        if not self.mean_loss:
            losses = {
                "loss_cls": F.binary_cross_entropy(
                    self.predict_probs_img(predictions, proposals),
                    gt_classes_img_oh,
                    reduction="sum",
                )
                / self.gt_classes_img_oh.size(0)
        ...
        losses = {
            "loss_cls": F.binary_cross_entropy(
                self.predict_probs_img(predictions, proposals), gt_classes_img_oh, reduction="mean"
            )
        }
...
class CSCOutputs(object):
    ...
    def csc_loss(self):
        ...
        if not self.mean_loss:
            return {
                ...
                + "loss_cls_pos": F.binary_cross_entropy(
                    pred_class_img_logits_pos, self.PL, reduction="sum"
                )
                * self.loss_weight
                / self.PL.size(0),
                self.prefix
                + "loss_cls_neg": F.binary_cross_entropy(
                    pred_class_img_logits_neg, self.NL, reduction="sum"
                )
                * self.loss_weight
                / self.NL.size(0),
            }
        ...
        return {
            self.prefix
            + "loss_cls_pos": F.binary_cross_entropy(
                pred_class_img_logits_pos, self.PL, reduction="mean"
            )
            * self.loss_weight,
            self.prefix
            + "loss_cls_neg": F.binary_cross_entropy(
                pred_class_img_logits_neg, self.NL, reduction="mean"
            )
            * self.loss_weight,
        }
