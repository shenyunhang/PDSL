...
from .meta_arch import GeneralizedMCNNWSL, GeneralizedRCNNWSL
...
class GeneralizedRCNNWithTTAAVG(nn.Module):
    ...
    def __init__(self, cfg, model, tta_mapper=None, batch_size=1):
        ...
        if isinstance(model, DistributedDataParallel):
            model = model.module
        assert isinstance(model, GeneralizedRCNNWSL) or isinstance(
            model, GeneralizedMCNNWSL
        ...
        self.is_ps = False
        if isinstance(model, GeneralizedMCNNWSL):
            self.is_ps = True
